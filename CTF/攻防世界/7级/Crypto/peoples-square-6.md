...
[题目地址](https://adworld.xctf.org.cn/challenges/details?hash=7dd6c57a-e04f-45b6-9d06-e2de96786ac9_2)

屑官方用别的队伍的[wp](https://github.com/p4-team/ctf/tree/master/2016-03-12-0ctf/peoples_square)机翻拿过来，根本看不懂。我自己尝试翻译一下，说不定能理解大佬的思路。
...
不难发现，这些代码实现的功能只是aes加密某些数据而已。RealEncrypt就是普通的aes，不过只有4轮——正常是10轮，（正是因为这个问题我们才得以攻击密文）。
...
encrypt是改动的地方，为4轮的aes。

怎么破解？我们需要用广为人知的[square attack](https://zhuanlan.zhihu.com/p/380860908),或者叫[integral cryptanalysis](https://en.wikipedia.org/wiki/Integral_cryptanalysis)。（太难了听都没听过）

想要理解这种攻击的原理，我们需要先简单了解一下aes。

我们知道aes基于128bit的块。这128 bit可以看作一个4*4字节的数组（其实就是矩阵了），然后我们对数组进行一些“操作”。

什么类型的操作呢？

1. SubBytes - 矩阵中的每个字节都替换为另一个字节，利用固定的关联表（associative table）（也可以说是替换箱（substitution box），缩写sbox）。给定以下矩阵：

输入矩阵： 1 2 3 4 6 7 8 5 11 12 9 10 16 13 14 15
...
ShiftRows - 矩阵的行向左偏移row_number大小。比如以下矩阵：
...
（第一行的row_number是0，我猜测跟索引有关系，所以不变。第二行是1，因此全部元素往左一格，超出界限的回到末尾，比如5。第三行就是2了，以此类推。）

MixColumns - 将矩阵列中的值相互混合（准确来说，把这些列看作多项式，然后乘以另一个固定的多项式，不过现在不重要）。重要的是列中最后得到的值仅取决于列中的其他值（What's important is that final values in column depends only on another values in that column）。
...
AddRoundKey - 现在为矩阵中的字节添加轮密钥（round key）。注意我们没有用平时的加法操作，而是异或（xor）操作（更严谨的理解：我们在有限域下操作，xor在这里是一种加法的类比。）(in technical words, that's because we are operating on galios field, and 'xor' is addition analogy there)
...
为什么要看这些理论？因为现在才是真正的攻击。

假设我们得到了256个由我们选择的明文的密文-不是乱选的，要让每个明文中的字节都是一致的，除了一个字节变换为所有可能的值。
...
重点：C表示我们指定的明文中不变的字节，X表示所有明文中变换的字节。

看看aes会怎样操作我们选择的密文：初始值。
...
前面提到了，第一个字节会变为所有可能的值，其他的字节都是不变的。
...
现在有些东西了。C的值在某个状态下不是完全相同了，不过在每一个明文都能得到相同的C值——这是很重要的地方。类似地，每个明文中的X字节也会得到所有可能的值。应该很清楚了，继续。
...
发生了什么？现在所有字节的取值都是任意可能值了，啥玩意啊？但可能是一件好事？

这意味着如果固定任意状态位置（比如第二行和第二列），然后将两者异或：（state_xx 表示我们选择的第xx个明文的状态）

```
state_01[2][2] ^ state_02[2][2] ^ state03_[2][2] ^ ... ^ stateFF[2][2] = 0
```

我们只会得到0。这是因为我们知道这些字节有所有可能的值，可以用随机顺序异或这些数字：
...
但是在第4轮后我们可以猜测key的一个字节来解密被加密数据的1个字节。从前面的式子可以得知，如果我们猜测的字节是正确的，连带着用这个字节解密的数据，一起异或得到的结果必然是0。

接着我们重复以上步骤16次——每次猜测key的一个字节。平均可以找到2个正确的值（1个完全正确，另一个256*1/256的误报几率）（对不起这一句我真的看不懂，乱翻的，给原文：On average we will find 2 good values (one correct, and 256 * 1/256 chance of false positive)）。我们要做的只是尝试所有的可能性，攻击复杂度为2^16，比2^128好多了。

我们还能简单地从任何轮密钥中复原主要密文的密钥。（We can also quite easily recover the master cipher key from any round key.）

这种攻击可以用于这道题吗？当然——如果我们选取256个明文，这些明文只有1个字节不同——‘i'的最低字节。（we can see that every one of them is differing only on ony byte - lowest byte of 'i'.，不是很懂这个i是啥意思）
