...
- `set *(char **)addr = value`
    ...
    - 以及32位下修改addr处的内存（4个字节）：`set *(unsigned int*)addr=value`
...
- interactive运行docker image： https://stackoverflow.com/questions/40854482/interactive-command-in-dockerfile
    ...
    - 如果是web题需要开放端口： `docker run -it --expose <container port> -p <container port>:<localhost port> <image id>`,参考 https://stackoverflow.com/questions/30109037/how-can-i-forward-localhost-port-on-my-container-to-localhost-on-my-host
- 从Container里拷贝文件到本机： https://stackoverflow.com/questions/22049212/copying-files-from-docker-container-to-host
    ...
    - libc等文件的路径： https://stackoverflow.com/questions/5925678/location-of-c-standard-library
