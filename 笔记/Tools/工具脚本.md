...
def PIEDebug(file,breakpoints,params=[]):
    ...
    global BASE
    sla("(gdb)",f"file {file}")
    sla("(gdb)",f"starti {' '.join(params)}")
    sla("(gdb)","info proc mappings")
    BASE=base=int(rls(5)[4].split(b'0x')[1][:-1],16)
...
def set_rip(rip):
    if not IS_PIE:
        sla("(gdb)",f"set $rip = {rip}")
    else:
        sla("(gdb)",f"set $rip = {BASE+rip}")
```
...
## python [pyzipper](https://github.com/danifus/pyzipper)爆破zip密码

使用pyzipper是因为这个模块支持v5.1及以上的zip，而python自带的zipFile模块不支持，archpr也是。

```python
import pyzipper
dict_path='dict.txt'
with pyzipper.AESZipFile('ctf.zip', 'r', compression=pyzipper.ZIP_DEFLATED, encryption=pyzipper.WZ_AES) as extracted_zip:
