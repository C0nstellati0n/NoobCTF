...
定理：f是环 $R_1$ 到 $R_2$ 的满同态，k=kerf，则有下列结论：
...
例1：有整数环Z和\<p\>，p为素数（刚才的例子已经证明过它是素理想了）。设N是Z的理想且\<p\> $\subset N$ ，\<p\> $\not ={N}$ 。故存在 $l\in N$ 但 $l\notin$ \<p\>。故l，p互素。则存在 $a,b\in Z$ ，使得al+bp=1。l和p都在N里面，因此 $1\in N$ 。这就导致 $\forall a\in Z,a=a\*1\in N$ ，整数环中的任何元素都在N里面 $\Rightarrow Z=N\Rightarrow$ \<p\>是极大理想。这个例子加上前面那个例子，告诉我们整数环里面由素数生成的主理想不仅是素理想，还是极大理想。

例2:R是域，{0}是R的极大理想。要证明这点，设N是R的理想，且 $N\not ={\{0\}}$ 。则存在 $a\in N$ 且 $a\not ={0}$ （这里提前说一下域，域的非零元全体构成一个群，且域满足交换除法）。于是有 $a^{-1}a=1\in N\Rightarrow \forall b\in R,b=b\*1\in N\Rightarrow R=N$ (1表示环的幺元)。

定理：设R是交换幺环，M为R的极大理想，当且仅当R/M是域。

证明：R/M是域，则{0}是R/M的极大理想。设A是R中包含M的理想 $\Rightarrow A/M$ 是R/M的理想（满同态将包含核的理想对应到理想） $\Rightarrow A/M=\{0\}$ 或A/M=R/M $\Rightarrow A=M$ 或A=R $\Rightarrow M$ 是极大理想。

反过来，设M是极大理想 $\Rightarrow$ R/M是域。有自然同态 $\pi:R\rightarrow R/M$ 。对于R/M的任意理想I，必存在包含M的R中的理想A,使得 $I=\pi(A)=A/M$ 。此时由M是极大理想，推出A=M或A=R。说明I={0}或I=R/M。说明{0}是极大理想。

由{0}是R/M的极大理想可推出R/M是域。事实上， $\forall a\in R/M$ 且 $a\not =0+M$ ，有 $0\subset$ \<a\> $\Rightarrow$ \<a\>=R $\Rightarrow 1\in$ \<a\> $\Rightarrow\exists a^{-1}\in R$ ，使得 $a^{-1}a=1$ 。又R是交换幺环，说明R是域。

结合起来就有了命题：{0}是交换幺环R的极大理想 $\Leftarrow$ R是域。

推论：交换幺环R的极大理想一定是素理想。

证： $I\rightarrow R$ 的极大理想 $\Leftrightarrow$ R/I是域 $\Rightarrow$ R/I是整环 $\Leftrightarrow$ I是素理想。

命题（同态基本定理第二条推广）：f是交换幺环R到R'的满同态。M是R中包含kerf的素（极大）理想。则f(M)是R'中的素（极大）理想。

证明： $R/M\simeq R'/f(M)$ 。M是素（极大）理想 $\Leftrightarrow$ R/M是整环（域） $\Leftrightarrow R'/f(m)$ 是整环（域） $\Leftrightarrow f(M)$ 是素（极大）理想。

## 23-2-4唯一析因环1

整数环里有个很重要的概念叫素数。推广一下，域P上的一元多项式也有不可约多项式。是否能把这些概念抽象出来？这节课我们就看推广到整环的情况。

首先要定义整除：设R是一个整环， $a,b\in R$ 。称a能被b整除，若存在 $c\in R$ ，使得a=bc。此时也称b是a的因子，记为b|a。若a不能被b整除，记 $b\not| a$ 。

单位群定义：设R为整环。用U表示幺半群 $R^{\*}=R-\{0\}$ 中的可逆元的集合。则U是交换群，称为R的单位群。U中的元素称为单位。对于整环来说，单位元一定是单位。 $\pm 1$ 都是单位。

例：集合 $Z[\sqrt{-1}]=\{a+b\sqrt{-1}|a,b\in Z\}$ 。定义加法、乘法 $\Rightarrow Z[\sqrt{-1}]$ 是整环 $\Rightarrow 1,-1,\sqrt{-1},-\sqrt{-1}$ 都是单位。

相伴定义：设 $a,b\in R$ 。若存在R中的单位u，使得a=ub，则称a与b相伴，记为a～b。

性质：
1. $\forall a\in R,a|a$ 。若 $a|b，b|c\Rightarrow a|c$ 。
2. u是单位，则 $u|a,\forall a\in R$ 。因为单位元可逆，任意元素a都可以写成 $a=(au^{-1})u$ 的形式。
3. 若a与b相伴，则存在单位u，使得b=au。 $u\in R^{\*}$ 是单位 $\Leftrightarrow u～1$
4. 相伴关系是R中的等价关系。且是 $R^{\*}$ （幺半群）中的同余关系。 $a～b\Leftrightarrow a=bu_1;c～d\Leftrightarrow c=du_1.u_1,u_2\in U\Rightarrow u_1u_2\in U.ac=(bd)(u_1u_2)\Rightarrow ac～bd$ （非规范写法）。

真因子定义：设 $a\in R^{\*}$ ，则单位和a的相伴元都是a的因子，称为a的平凡因子。若b|a，但是 $a\not|b$ ，则称b为a的真因子。

不可约元定义：设 $a\in R^{\*}-U$ 。若a没有非平凡的真因子，则称a为不可约元。若a又非平凡的真因子，则称a为可约元。

例1:在整数环中，U={1,-1}。若 $a～b\Leftrightarrow a=\pm b$ 。因而a为不可约元素 $\Leftrightarrow$ a为素数或者负素数。

例2:设P是域，P[x]是P上的一元多项式环。 $U=P^{\*}=P-{0}$ 。 $f(x)～g(x)\Leftrightarrow f(x)=cg(x)$ 。因而f(x)为不可约元素 $\Leftrightarrow f(x)$ 是不可约多项式。

素元素定义：若 $p\in R^{\*}-U$ 。且由 $p|ab\Rightarrow p|a$ 或p|b，则称o是素元素。

命题：素元素是不可约元素。

证明：设p是素元素，a是p的因子。则存在 $b\in R^{\*}$ 使得p=ab。即p|ab $\Rightarrow p|a$ 或p|b。若p|a，由a|p $\Rightarrow$ a不是真因子。若p|b，则存在 $c\in R^{\*}$ 使得b=pc,则p=pac $\Rightarrow ac=1\Rightarrow$ a是单位 $\Rightarrow$ a是平凡因子。说明p一定是不可约的。

那不可约元素是否一定是素元素？并不是。看一个反例就知道了。

## 24-2-4唯一析因环2

反例：集合 $Z[\sqrt{-5}]=\{a+b\sqrt{-5}|a,b\in Z\}$ 。定义普通的加法和乘法 $\Rightarrow$ R是一个整环。任意元素 $\alpha=a+b\sqrt{-5}$ 的共轭是 $\overline{\alpha}=a-b\sqrt{-5}$ 。定义函数 $N(\alpha)=\alpha\overline{\alpha}=a^2+5b^2\geq 0$ ，等号成立 $\Leftrightarrow \alpha=0.\forall \alpha,\beta\in R,N(\alpha\beta)=N(\alpha)N(\beta)$ 。考虑3，可以证明3是不可约元素。设 $\alpha=a+b\sqrt{-5}$ 是3的因子，故有 $\beta\in R^{\*}$ 使得 $3=\alpha\beta$ 。就有 $9=N(3)=N(\alpha)N(\beta)$ 。9的因子只有1，3，9，于是只有3种取法：

1. $N(\alpha)=1\Rightarrow \alpha=\pm 1\Rightarrow \alpha$ 是单位。
2. $N(\alpha)=3\Rightarrow a^2+5b^2=3$ ，无解，不存在。
3. $N(\alpha)=9\Rightarrow N(\beta)=1\Rightarrow \beta=\pm 1\Rightarrow 3～\alpha\Rightarrow\alpha$ 是3的平凡因子。

总结以上3种情况，3是不可约元素。但3是否是素元素？ $3|9=(2+\sqrt{-5})(2-\sqrt{-5})$ 。假如3是素元素的话，它必定整除其中之一。设 $3|(2+\sqrt{-5})$ ,则存在 $\beta\in R$ ，使得 $2+\sqrt{-5}=3\*\beta\Rightarrow N(\beta)=1\Rightarrow\beta=\pm 1\Rightarrow 2+\sqrt{-5}=\pm 3$ ，不可能。说明 $3\not| 2+\sqrt{-5}$ 。同理， $3\not| 2-\sqrt{-5}$ 。综上所述，3不是素元素，存在不是素元素的不可约元。

唯一析因环定义：如果整环满足下列条件：

1. 有限析因条件： $\forall a\in R^{\*}-U$ ，a可分解为有限个不可约元素的乘积，即有不可约元素 $P_i(1\leq i\leq r)$ ，使得 $a=P_1P_2...P_r$ 
2. 若 $a\in R^{\*}-U$ ，有两种不可约元素乘积的分解： $a=P_1P_2...P_r=q_1q_2...q_s$ ，则有r=s。而且适当交换因子顺序，可使得 $P_i～q_1,1\leq i\leq r$ 。

则称环R是唯一析因环。整数环，数域上的一元多项式环都是唯一析因环。肯定存在不是唯一析因环的整环，比如刚才看的 $Z[\sqrt{-5}]$ 。里面的 $9=3\*3=(2+\sqrt{-5})(2-\sqrt{-5})$ 。无论 $(2+\sqrt{-5})$ 还是 $(2-\sqrt{-5})$ ，它们都是不可约元素却不与3相伴。不满足定义的第二点。

定理：唯一析因环R中的不可约元素是素元素。

证明：设 $p\in R^{\*}-U$ 是不可约元素,p|ab。要证p|a或p|b。由p|ab，一定存在 $c\in R$ ，使得ab=pc。

1. a,b其中之一是单位，不妨设为b，则 $a=pcb^{-1}$ ，即p|a，结论成立。
2. $a,b\in R^{\*}-U\Rightarrow c\in R^{\*}-U$ 。关于c的取值现在证明：首先 $c\not ={0}\Rightarrow c\in R^{\*}$ 。若 $c\in U\Rightarrow pc～p\Rightarrow$ p是可约的。可是前提条件就是p是不可约元素，说明c只能这么取值。回到原本的证明。R是唯一析因环，存在 $R^{\*}-U$ 中的不可约元素： $\{P_i\}_{1\leq i\leq t},\{q_i\}_{1\leq i\leq r},\{q_i'\}_{1\leq i\leq s}$ 使得:
- $c=p_1p_2...p_t$
- $a=q_1p_2...p_r$
- $b=q_1'q_2'...q_s'$

$\Rightarrow p_1p_2...p_t=q_1p_2...p_r=q_1'q_2'...q_s'$ 。由分解唯一性，p必定与某个 $q_i$ 或 $q_j'$ 相伴。若 $p～q_1$ ，则 $p=q_i\epsilon,\epsilon\in U$ 。故 $a=q_1...q_{i-1}p\epsilon^{-1}q_{i+1}...q_r=p(q_1...q_{i-1}\epsilon^{-1}q_{i+1}q_r)\Rightarrow p|a$ 。若 $p～q_j'\Rightarrow p|b$ ，说明p是素元素。

定理：整环R满足有限析因条件且每个不可约元素都是素元素，则R是唯一析因环。

## 25-2-4唯一析因环3

证明上节课提到的最后一个定理。设 $a\in R^{\*}-U$ ,则 $a=p_1p_2...p_r$ 。其中 $p_i,1\leq i\leq r$ 是不可约元素。设 $a=q_1q_2...q_s$ 是a的另一种分解， $q_i,1\leq i\leq s$ 是不可约元素。若r=1， $a=p_1=q_1q_2...q_s$ 。若s大于等于2， $a=p_q=q_1(q_2...q_s)\Rightarrow p_1$ 是可约的。与之前的假设矛盾，说明s=1， $p_1=q_1$ 。这样的结果推广到任何r=k-1的情况上，定理都成立。考虑r=k。 $a=p_1p_2...p_k=q_1q_2...q_s\Rightarrow p_1|q_1q_2...q_s\Rightarrow$ 一定存在 $1\leq j\leq s$ ，使得 $p_1|q_j$ 。又 $p_1,q_j$ 是不可约元素，得到 $p_1=\epsilon q_j$ 。不妨设j=1，即 $p_1=\epsilon q_1\Rightarrow \epsilon p_2p_2...p_k=q_2q_3...q_s,\epsilon$ 是单位。由归纳假设，k-1=r-1=s-1，且适当交换顺序，可使 $p_i～q_i,2\leq i\leq k$ 。最终推出来R是唯一析因环。

公因子定义：设 $a_1,a_2...a_n\in R$ 。若 $c\in R$ 同时整除 $a_1...a_n$ ，则称c是 $a_1...a_n$ 公因子。若 $a_1,a_2...a_n$ 的公因子d能被 $a_1...a_n$ 的任何一个公因子整除，则称d是 $a_1...a_n$ 的最大公因子。

定理：设R是唯一析因环， $a,b\in R$ ，则a，b的最大公因子存在，而且a，b的最大公因子相伴。

证明：首先证明最大公因子的存在性。若a，b有一个是零，不妨设为a，则b是最大公因子。若a，b有一个是单位，不妨设为a，则a是最大公因子。若 $a,b\in R^{\*}-U$ ，则a，b有分解： $a=q_1q_2...q_r,b=q_1'q_2'...q_s'$ 。其中 $q_i,q_i'$ 都是不可约元素。把上述分解中的相伴元写在一起，可以统一写成： $a=\epsilon_ap_1^{k_1}...p_n^{k_n},b=\epsilon_bp_1^{l_1}...p_n^{l_n}$ 。其中 $\epsilon_a,\epsilon_b$ 是单位， $k_1,l_j\geq 0,p_1...p_n$ 互不相伴。令 $m_i=min\{k_i,l_i\},i=1,2,..,n$ 。令 $d=p_1^m...p_n^{m_n}\Rightarrow d|a,d|b$ 。这个d就有最大公因子的味道了，如何证明？假设c是a，b的公因子，此时c不等于0。若c是单位，则c｜d。若 $c\in R^{\*}-U$ ，则 $c=p_1'p_2'...p_t'$ 。其中 $p_i'$ 是不可约元素，故为素元素。由定义知， $p_i'$ 与某个 $p_j$ 相伴。c就可以写为 $\epsilon_cp_1^{h_1}...p_n^{h_n}$ 。由c｜a，且 $p_1...p_n$ 互补相伴，得到 $h_i\leq k_i$ 。同理， $h_i\leq l_i$ 。综合得到 $h_i\leq m_i\Rightarrow c|d$ 。得到d是最大公因子。

然后是最大公因子相伴的证明。设d，d'都是最大公因子，则d|d',d'|d。则存在u，v使得d=ud',d'=vd。有以下情况：
1. d，d'有一个是0，则d=d'=0
2. 都不为0。 $d=uvd\Rightarrow uv=1\Rightarrow$ d与d'相伴。

## 26-2-5主理想整环

主理想整环定义：若交换幺环的每个理想都是主理想，则称此环是主理想环。若主理想环是整环，则称此环为主理想整环。

例:整数环Z：整环。设I是Z的理想。则存在 $m\in I$ ，使得m=min{ |k| $\mid k\in I,k\not ={0}$ }。现设 $b\in I$ ，若b=0=0\*m。若b不是0，b=qm+r，其中 $0\leq r$ < m， $r=b-qm\in I$ 。m已经是最小的了，不存在比m更小的r，除非r等于0，推出b=qm $\Rightarrow I=$ \<m\> $\Rightarrow Z$ 是主理想整环。

引理1:设R是主理想整环，若R中的一个序列： $a_1,a_2,a_3...$ ，里面每一个元素都是前面一个元素的真因子，则此序列是有限序列。

证明：用这一序列作理想： < $a_1$ >,< $a_2$ >,< $a_3$ >...由于 $a_{i+1}$ 是 $a_i$ 的真因子，故 < $a_i$ > $\subseteq$ < $a_{i+1}$ > ,i=1,2...（ $a_2|a_1,a_1=a_2\*u,Ra_1=Ra_2\*u=Ru\*a_2$ )。令I是所有理想的并，令 $a,b\in I\Rightarrow a\in$ < $a_i$ >, $b\in$ < $a_j$ >。不妨设 $i\leq j\Rightarrow a,b\in$ < $a_j$ > $\Rightarrow a-b\in$ < $a_j$ > $\subseteq I$ 。 $\forall r\in R,ra\in$ < $a_i$ > $\subseteq I\Rightarrow I$ 是R的理想。则存在 $d\in R$ ，使得I=< d >。有 $d\in I\Rightarrow$ 存在n，使得 $d\in$ < $a_n$ > $\Rightarrow a_n|d$ 。断定 $a_n$ 是序列的最后一项，若不然， $a_{n+1}\in$ < $a_{n+1}$ > $\subseteq I=$ < d > $\Rightarrow d|a_{n+1}\Rightarrow a_n|a_{n+1}$ 。与 $a_{n+1}$ 是 $a_n$ 的真因子矛盾。说明序列一定是有限序列。

引理2:设R是一个主理想整环， $p\in R^{\*}-U$ 是不可约元素。则 < p >是极大理想。

证明：设I是R的理想且< p > $\subseteq I$ 。又R是主理想整环，则存在 $a\in R$ ,使得I=< a >。故 $p\in$ < a >。从而存在 $r\in R$ ,使得p=ra。而p是不可约元素，说明a是单位或a～p。若a是单位，I=R。若a～p，I=< p > $\Rightarrow$ < p >是极大理想。

定理：主理想整环是唯一析因环。

证明：首先证明主理想整环满足有限析因条件。设 $a\in R^{\*}-U$ ，不满足有限析因条件。则a可约，进而存在b，c，使得a=bc。其中b，c是a的真因子。且b，c中至少有一个不满足有限析因条件。那么那个不满足有限析因条件的元素又能分解，分解结果中又至少有一个不满足有限析因条件。这样操作下去，有无穷序列 $a,a_1,a_2...$ ，且每个元素是前一个元素的真因子。但这与引理1矛盾。说明R满足有限析因条件。

接下来证R的不可约元素是素元素。设 $p\in R^{\*}-U$ 是不可约元素，且p｜ab。考虑R/< p >，由引理2，R/< p >是域。又 $ab\in$ < p > $\Rightarrow \overline{a}\*\overline{b}=\overline{ab}=\overline{0}\Rightarrow \overline{a}=\overline{0}$ 或 $\overline{b}=\overline{0}\Rightarrow a\in$ < p >或 $b\in$ < p > $\Rightarrow p|a$ 或p｜b。说明p是素元素。有限析因条件+不可约元素是素元素，得到R是唯一析因环。

## 27-2-6欧几里得环1

欧几里得环定义：设R是整环。若存在 $R^{\*}$ 到 $N\cup$ {0}上的映射 $\delta$ ,使得 $\forall a,b\in R,b\not ={0}$ ，存在 $q,r\in R$ ，满足：a=qb+r。其中r=0，或 $\delta(r)$ < $\delta(b)$ 。则称R是欧几里得环。

例1：整数环Z是欧几里得环。 $\delta\rightarrow Z^{\*}\rightarrow N\cup$ {0}, $\delta(m)=|m|$ 。

例2:数域p上的一元多项式环p[x]。 $\delta:p[x]^{\*}\rightarrow N\cup$ {0}, $\delta(f(x))=deg f(x),f(x)\not ={0}$ (零多项式不定义次数)。

例3: $Z[\sqrt{-1}]=$ { $a+b\sqrt{-1}|a,b\in Z$ }是欧几里得环。

令 $\delta(a+b\sqrt{-1})=a^2+b^2$ 。易证 $\delta(\alpha\beta)=\delta(\alpha)\delta(\beta)$ 。设 $\beta\not ={0},\beta\in Z[\sqrt{-1}].\beta^{-1}\in Q[\sqrt{-1}].\alpha\beta^{-1}=u+\gamma\sqrt{-1}.u,r\in Q$ 。于是存在 $c,d\in Z$ ，使得 $|c-u|\leq\frac{1}{2}.|d-r|\leq\frac{1}{2}$ 。令 $\epsilon=u-c,\eta=r-d$ 。则 $|\epsilon|\leq\frac{1}{2},|\eta|\leq\frac{1}{2}$ 。且 $\alpha=\beta(c+\epsilon)+(d+\eta)\sqrt{-1}=\beta q+r$ 。其中 $q=c+d\sqrt{-1},r=\alpha-\beta q\in Z[\sqrt{-1}]$ 。又若 $r\not ={0},\delta(r)=\delta(\beta(\epsilon+\eta\sqrt{-1}))=\delta(\beta)\*\delta(\epsilon+\eta\sqrt{-1})=\delta(\beta)(\epsilon^2+\eta^2)\leq\delta(\beta)(\frac{1}{4}+\frac{1}{4})\leq\overline{o}\beta$ 。总结以上， $Z[\sqrt{-1}]=$ 是欧几里得环。

定理：欧几里得环是主理想整环，进而是唯一析因环。

证明：设I是欧几里得环R的理想， $\delta$ 是定义中的映射。若I=0，则I=< 0 >。若 $I\not ={0}$ ，则集合 { $\delta(x)|x\in I,x\not ={0}$ }中一定存在最小值。设 $a\in R^{\*}\cap I$ ，且 $\delta(a)$ 取到最小值。即 $\forall x\in I,x\not ={0}$ ,有 $\delta(x)\geq\delta(a)$ 。对 $\forall b\in I$ ，存在q，r使得b=qa+r。若r不为0， $\delta(r)$ < $\delta(a)$ 。但是， $r=b-qa\in I$ ,且 $\delta(r)$ < $\delta(a)$ 。矛盾。说明r=0，则b=qa $\Rightarrow I=$ < a > $\Rightarrow R$ 是主理想整环。

命题：R是主理想整环。 $a,b\in R$ ，d为a，b的最大公因子。则存在 $u,v\in R$ ，使得d=ua+vb。

证明：存在 $d'\in R$ ，使得< d' >=< a,b > $\Rightarrow a,b\in$ < d' > $\Rightarrow d'|a,d'|b$ $\Rightarrow$ d'是a，b的公因子。又 $d'\in$ < a,b >，故存在 $u',v'\in R$ ,使得d'=u'a+v'b。于是c是a，b的公因子，推出c|u'a+v'b=d' $\Rightarrow$ d'是a，b的最大公因子 $\Rightarrow$ 存在 $\epsilon\in U,d=\epsilon d'=\epsilon u'a+\epsilon v'd$ 。这里的 $\epsilon u'=u,\epsilon v'=v$ 。

定义：a，b互素，如果1是a，b的最大公因子。这里的1表示单位元。

命题：设R是主理想整环。a，b互素 $\Leftrightarrow\exists u,v\in R$ ，使得ua+vb=1。可以看作是上个命题的推论，不过有些差别。

证明可由上个命题直接得到。不一样的地方在于反过来也成立。设d是a，b的最大公因子。d｜ua+vb=1。故d～1。即1是a，b的最大公因子，a，b互素。

对于欧几里得环R，设 $a,b\in R^{\*}$ ，不妨设 $\delta(a)\geq\delta(b)$ 。令 $a_1=a,a_2=b.a_1=qa_2+a_3$ 。其中 $a_3=0$ 或 $\delta(a_3)$ < $\delta(a_2)$ 。若 $a_3=0\Rightarrow a_2$ 是 $a_1,a_2$ 的最大公因子。若 $a_3\not ={0},a_2=q_1a_3+a_4$ 。其中 $a_4=0$ 或 $\delta(a_4)$ < $\delta(a_3)$ 。重复上面的过程， $\delta(a_1)\ge\delta(a_2)\ge\delta(a_3)\ge\delta(a_4)\ge...$ 在有限步骤终止。有 $a_n\not ={0}$ 而 $a_{n+1}=0\Rightarrow a_n$ 是a，b的最大公因子。这就是欧几里得环里的辗转相除。

## 28-2-6欧几里得环2

如果让全体整环为A，唯一析因环为B，主理想整环为C，欧几里得环为D。那么它们之间的包含关系是 $D\subset C\subset B\subset A$ 。它们之间是真包含吗？
1. A包含B但不等于B，真包含关系。之前看的例子 $Z[\sqrt{-5}]=\{a+b\sqrt{-5}|a,b\in Z\}$ 是整环，但不是唯一析因环。
2. Z[x]是唯一析因环。若Z[x]是主理想整环，则存在 $g(x)\in Z[x]$ ，使得< g(x) >=< $2,x^2+1$ > $\Rightarrow 2\in$ < g(x) > $\Rightarrow g(x)|2\Rightarrow g(x)=\pm 1$ 或 $\pm 2$ 。又 $g(x)\in$ < $2,x^2+1$ > 。故存在 $u(x),v(x)\in Z[x]$ ，使得 $g(x)=u(x)\*2+v(x)\*(x^2+1)\Rightarrow g(1)=2(u(1)+v(1))\Rightarrow g(x)=\pm 2$ 。但 $\pm 2\not|x^2+1$ 。即 $x^2+1\notin$ < g(x) >,矛盾。说明Z[x]不是主理想整环。B和C也是真包含关系。
3. 环R={ $a+\frac{b}{2}(1+\sqrt{-19})|a,b\in Z$ }。可以证明R是主理想整环但不是欧几里得环。C和D也是真包含关系。

设F是域，x为一个文字（符号），则由形如： { $a_nx^n+a_{n-1}x^{n-1}+...+a_1x^1+a_0|n\in N\cup \{0\},a_i\in F$ }的所有元素组成集合F[x]成为F上的一元多项式环。在F[x]上定义加法，乘法如下：

$\forall f(x),g(x)\in F[x],f(x)=\sum _{i=0}^n a_nx^i,g(x)=\sum _{i=0}^m b_ix^i$

加法：f(x)+g(x)为 $x^i$ 的系数相加作为新的系数。

乘法： $f(x)\*g(x)=\sum _{k=0}^{m+n}c_kx^k(c^k=\sum _{i+j=k}a_ib_j)$

易验证:F[x]是交换幺环。对上述的f(x), $a_n\not ={0}$ ，则称n是非零多项式f(x)的次数，记为deg f(x)（不定义0多项式的次数）。有deg(f(x)\*g(x))=deg(f(x))+deg(g(x)) $\Rightarrow$ F[x]是无零因子环 $\Rightarrow$ F[x]是整环。若 $g(x)\not ={0},g[x]\in F[x]$ ，则对任意 $f(x)\in F[x]$ ，存在 $q(x),r(x)\in F[x]$ ，使得f(x)=q(x)g(x)+r(x)。其中r(x)=0,或者deg r(x)小于deg g(x)。且这样的q(x),r(x)唯一。现定义 $\delta:F[x]^{\*}\rightarrow N\cup \{0\},\delta(f(x))=$ deg f(x) $\Rightarrow F[x]$ 是欧几里得环。

设I是F[x]的理想。一定存在 $f(x)\in F[x]$ ,I=< f(x) >。若I不为0，则f(x)不为0。设f(x)可约，即存在 $f_1(x),f_2(x)\in F[x]$ 使得 $f(x)=f_1(x)f_2(x)$ ，且 $deg(f_1(x)),deg(f_2(x))$ 均大于0.则有 < f(x) > $\subset$ < $f_1(x)$ >，且< f(x) > $\subset$ $f_2(x)$ 。因为两个都是真因子，所有肯定是真包含。又因为多项式次数大于0，不可能生成0次多项式，所以 < f(x) >不是F[x]的极大理想。反之，f(x)是F[x]的不可约多项式 $\Rightarrow$ < f(x) >是F[x]中的极大理想 $\Leftrightarrow$ F[x]/< f(x) >是一个域。

总结以上就有了一条定理：设 $f(x)\in F[x]$ 是不可约多项式，则< f(x) >是F[x]的极大理想。进而F(x)/< f(x) >是域。

## 29-2-7环上的一元多项式1

设R为环，考虑形如 $f=\{a_0,a_1,a_2,...\},a_i\in R$ 的序列，其中只有有限个 $a_i$ ，不为0。令S为所有这种序列组成的集合，在S上定义两种运算，加法与乘法，如下：对于
...
f+g={ $a_0+b_0,a_1+b_1,a_2+b_2$ ...},<br>
fg={ $a_0b_,a_0b_1+a_1b_0,a_0b_2+a_1b_1+a_2b_0$ ...}={ $c_k$ },

其中 $c_k=\sum _{i+j=k}a_ib_j,k=0,1,2...$

命题2.7.1：S在上述加法和乘法下构成一个环。若R为幺环/交换环，则S也是幺环/交换环。S的零元是{0,0,0...}，因此 $f=\{a_0,a_1,a_2,...\}$ 的负元是 $-f=\{-a_0,-a_1,-a_2,...\}$ 。如果I是R的幺元，则I'={1,0,0...}是S的幺元。只要R是环，就能定义出S。

今后我们将S中的元素称为R上的多项式，或者系数在R上的多项式。如果 $f=\{a_0,a_1,a_2,...\}$ 是非零多项式（即至少有一个 $a_i$ 非零）且n是使得 $a_n\not ={0}$ 的最大整数，则称f的次数是n。记f的次数为deg f。不定义零多项式的次数。若deg f=n，则称 $a_0,a_1,...a_n$ 为f的系数。 $a_n$ 为f的首项系数。首项系数为1（这时R为幺环）的多项式称为首一多项式。

命题2.7.2:若 $f,g\in S$ 非零，deg f=m，deg g=n。则：
1. f+g=0或deg(f+g) $\leq$ max(m,n)
2. fg=0或deg(fg) $\leq$ deg f+deg g。等号成立当且仅当f的首项系数 $a_m$ 与 $b_n$ 的首项系数的乘积 $a_mb_n$ 不为零。特别地，若R为整环，则S也是整环。

定义R到S的映射 $\phi:n\rightarrow$ {a,0,0,...}，则显然 $\phi$ 是环的单同态。由此R可以看成S的子环。若R是整环，则整环S的所有单位就是R的所有单位。

下设R为幺环，令r为下列多项式：r={0,1,0,0...}。则易见对非负整数m和 $a\in R$ 有 $ax^m=\{c_i\}$ ，其中 $c_i=0,i\not ={m};c_m=a$ 。因此任何n次多项式 $f=\{a_i\}$ 可以写成 $f=a_0+a_1x+a_2x^2+...+a_nx^n$ 。这样任何多项式都可以写成我们熟悉的r的多项式。通常我们将r称为一个因变量，而将S中的元素称为R上的一元多项式。而S本身记为S=R[x],称为环R上的一元多项式环。

下面我们换一个角度来理解多项式坏．设 $R_1$ 是幺环. R是 $R_1$ 的子环,且包含 $R_1$ 的么元.若 $f(x)=a_0+a_1x+a_2x62+...+a_nx^n$ 是R[x]上的一个一元多项式，则对任何 $a\in R_1$ ,令 $f(a)=a_0+a_1a+a_2a^2+...+a_na^n$ ,称为f(x)中用a代入所得的值.这样R上的任何一个一元多项式都可以看成 $R_1$ 上的取值 $R_1$ 上的一个函教.而对于固定的 $R_1$ 中的元素a，映射 $a\rightarrow f(a)$ 是一个R[x]到 $R_1$ 的同态。

值得注意的是，当我们将R上多项式看成 $R_1$ 的函致的时候,有可能不同的多项式导出一样的函致。例如，如果 $R=\{c_1,c_2,...c_n\}$ 是一个有限幺环，而且 $R_1=R$ 。则多项式 $f(x)=(x-c_1)(x-c_2)...(x-c_n)$ 是非零的。但是 $f(a)=0,\forall a\in R_1$ 。另一方面，确实存在这样的环 $R\subset R_2$ ，使得任何两个R[x]中不同的多项式作为 $R_2$ 上的函效也是不同的。例如取 $R_2=R[x]$ 即可。事实上，对于因定的多项式 $f\in R[x]$ ,定义的函数是 $g(x)\rightarrow f(g(x))$ 。因对于 $f\not ={f_1},f(x)\not ={f_1(x)}$ 。因此f与 $f_1$ 在 $x\in R_2$ 处的取值就是不一样的。

如果多项式 $f=a\in R$ ，那么将f看成 $R\subset R_1$ 上的函数的时候，它在任何点处取值都等于a。通常将R中的元素也称为常数多项式。值得注意的是，可能存在这样的多项式 $g\in R[x],g\notin R_1$ ，但是 $g(y),\forall g\in R_2$ 的取值却是常数。这时我们并不将g称为常数多项式。

用函数的观点来看多项式可以引出环上的多项式环的另一种（等价）定义。假定 $R,R_1$ 如上，固定一个元素 $a\in R_1$ ,则a定义了R[x]到 $R_1$ 的一个环同态 $\phi:\phi(f)=f(a)$ 。在这一同态下，R[x]的像构成 $R_1$ 的一个子环，记为R[a]。因此：

$R[a]=\{a_0+a_1a+a_2a^2+...+a_ma^m|a_i\in R,m\geq 0\}$

R[a]也可以看成是 $R_1$ 中包含R及a的最小子环。