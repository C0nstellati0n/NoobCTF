...
G有限，设 $x...x_n$ 为G的所有不同共轭类的代表元。 $G=\cup{n}_{i=1} C_{x_i}$ . $|G|=\sum^{n}_{i=1}|C_{x_i}|=\frac{n}{2}_{i=1}\frac{|G|}{|C(X_i)|}.C_{X_i}～G/C(X_i)$ 
...
定义：若在G中存在子群H，使得 $H\cap N=\{1\}$ (平凡元)， $HN=\{hn|h\in H,n\in N\}=G$ 。则称G是H与N的半直积，记为 $G=H\ltimes N(N\rtimes H)$ 或 $G=B\ltimes A$ 

定义2：若存在 $\delta:B\rightarrow G$ 群同态满足 $\delta(B)\cap N=\{1\}$ 且合成映射 $\mu\delta:B\rightarrow G\rightarrow B$ 为恒等映射。
...
定义：如果上述的H是G的正规子群，则称G为H与N的（内）直积。至于为什么这回没有“半”了，因为若 $\forall h\in H,n\in N,hn=nh\Rightarrow H\lhd G$ 。重点在于hn=nh，半直积里面就不一定了。