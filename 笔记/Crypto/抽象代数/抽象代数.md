...
2. E/F为代数扩张，若 $\forall \alpha\in E,\exist\space Irr(\alpha,F)\in F[x]$ 。称 $\alpha$ 可分若 $Irr(\alpha,F)$ 可分。否则称 $\alpha$ 不可分。
...
定理： $p(x)\in F[x]$ 不可约。
1. p(x)不可分，则p'(x)=0
2. $\exist h(x)\in F[x]$ 是可分不可约多项式，使得 $p(x)=h(x^p)$
...
定理：设 $E=F(\alpha_1...\alpha_n)$ 为F的可分扩张，则 $\exist\theta\in E$ 使得 $E=F(\theta)$
