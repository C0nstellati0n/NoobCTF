...
举个例子，有 $y=x^2$ 。若将 $x_0$ 增加个 $\Delta x$ ，其 $\Delta y$ 为 $(x_0+\Delta x)^2-x_0^2=2x_0\Delta x+(\Delta x)^2$ 。容易看出当 $\Delta x$ 很小时， $(\Delta x)^2$ 也会很小，故为一个高阶无穷小，记做 $O(\Delta x)$ 。如果把 $2x_0$ 看成一个整体，记为A，那么 $\Delta y=A\Delta x+O(\Delta x)$ , $A\Delta x$ 为线性主部

引出微分定义：y=f(x)在某个范围内有定义， $x_0\rightarrow x_0+\Delta x$ .那么函数值变化量的精确值为 $\Delta y=f(x_0+\Delta x)-f(x_0)$ 。若其可表示成 $\Delta y=A\Delta x+O(\Delta x)$ 的话（A不依赖 $\Delta x$ ，只与x有关，相对于 $\Delta x$ 来说可以看作是常数），就叫y=f(x)在 $x_0$ 处可微。线性主部记为 $dy=A\Delta x$ ，为变化量的近似值，里面的A就是导数； $\Delta x$ 就是dx
