...
二重积分常用体积来定义。如果f(x,y)是区域D上的有界函数。将区域D任意分成n个区域 $\Delta\sigma_1...\Delta\sigma_n$ ,并在每个 $\Delta\sigma_i$ 上任取一点 $(\xi_i,\eta_i)$ 作乘积 $f(\xi_i,\eta_i)\Delta\sigma_i$ 。当 $\lambda\rightarrow 0$ 时( $\lambda$ 表示每个区域的直径，即区域内距离最远的两点之间的距离)，若 $lim_{\lambda\rightarrow 0}\Sigma^n_{i=1}\Delta\delta_i f(\xi_i,\eta_i)=\int_D\int f\left(x,y\right)d\sigma$ 存在,就叫这个函数的二重积分。 $d\sigma$ 叫面积元素，指最开始划分区域时那些 $\Delta\sigma_i$ 。不过题目常考直角坐标系和极坐标系，所以面积元素不常见，取而代之的是dx，dy；dr, $d\theta$

虽然定义上 $\Delta\sigma_i$ 可以随便分，但是真随便分了不好算。如果是在直角坐标系上分，就按x轴画直线，y轴画竖线，形成若干个小长方形。它的长是 $\Delta x_i$ ，宽是 $\Delta y_j$ 。那么一个小区域 $\Delta\sigma_i=\Delta x_i\Delta y_j$ 。求二重积分的公式就是 $\int_D\int f\left(x,y\right)dxdy$
