...
FSOP为File Stream Oriented Programming的缩写，通常认为是通过伪造常见文件结构来劫持程序控制流的一种方法。由于不同libc版本的具体实现细节不同，FSOP的具体利用方式也不同。本文将基于2.35及以上版本的libc，尝试总结出简单直白的用法，不涵盖具体原理。

第一种做法来自nobodyisnobody的[总结](https://github.com/nobodyisnobody/docs/tree/main/code.execution.on.last.libc)，libc版本：2.38
...
不过我复现成功了House of Apple2里提到的`_IO_wfile_overflow`调用链，libc版本：2.35
...
logleak("fake_wide_data_addr",fake_wide_data_addr)
logbase()
